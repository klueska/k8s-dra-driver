...
func (m *ComputeDomainManager) NewSettings(ctx context.Context, domain string) *ComputeDomainDaemonSettings {
...
func (s *ComputeDomainDaemonSettings) GetCDIContainerEdits(devRoot string, info *nvcapDeviceInfo) (*cdiapi.ContainerEdits, error) {
	cd, err := s.manager.GetComputeDomain(context.Background(), s.domain)
	if err != nil {
